...
void PostProcess::SetBoxInfo_v2(size_t index, BBox &box) {
    float *boxBuff = static_cast<float *>(outputBox_);
    boxBuff += (index * sizeof(float));
    float w = boxBuff[2] * xScale_;
    float h = boxBuff[3] * yScale_;
    box.w = boxBuff[0] * xScale_ + w / 2;
    box.h = boxBuff[1] * yScale_ + h / 2;
    box.x = boxBuff[0] * xScale_ - w / 2;
    box.y = boxBuff[1] * yScale_ - h / 2;
}
...
void PostProcess::NMS(std::vector <BBox> &boxes, std::vector <BBox> &result) {
    result.clear();
...
void PostProcess::DrawBoundBoxToImage(const std::vector <BBox> &result) {
...
Result PostProcess::Process(std::vector <BBox> &result)
{
    std::vector <BBox> boxes;
    float *classBuff = static_cast<float *>(outputClass_);
    for (size_t i = 0; i < kModelOutputBoxNum; ++i) {
